...
interface TrnRepository : CrudRepository<Trn, String> {
    @Query("select t from Trn t where "
            + "t.portfolio.id =?1 and "
            + "t.tradeDate <= ?2")
    fun findByPortfolioId(portfolioId: String, tradeDate: LocalDate, sort: Sort): Collection<Trn>
    fun findByPortfolioIdAndAssetId(portfolioId: String, assetId: String, sort: Sort): Collection<Trn>
    ...
    fun findByPortfolioIdAndAssetIdUpTo(
            id: String, assetId: String, tradeDate: LocalDate): Collection<Trn>
    ...
    fun findExisting(portfolio: String,
                     asset: String,
                     trnType: TrnType,
                     tradeDate: LocalDate,
                     endDate: LocalDate): Collection<Trn>
}