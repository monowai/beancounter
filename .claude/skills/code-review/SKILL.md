...
- **Path aliases**: `@lib/*` → `src/lib/utils/*`, `@components/*` → `src/components/*`, `@hooks/*` → `src/hooks/*`, `@test-fixtures/*` (per `tsconfig.json` paths and bc-view CLAUDE.md "Path Aliases" section). For NEW code in a diff, flag any relative import (`./X`, `../X`, `../../X`) that resolves under one of these alias roots — including same-directory siblings and `__tests__/../X` patterns. Don't flag pre-existing relative imports outside the diff hunks (CI churn). Recurring CodeRabbit "nitpick" the local skill missed historically — treat as 🟡, not ⚪, because consistency is the rule.
- **`@lib/*` vs `@utils/*` — `@lib/api/*` is BANNED**: both aliases resolve to `src/lib/utils/*` in `tsconfig.json`, but `@lib/api/*` FAILS at Next.js runtime (returns 500). bc-view ships a pre-commit hook (`scripts/check-imports.js`) that hard-fails any `@lib/api` import. **Flag `@lib/api/*` as 🔴**. For API helpers (e.g., `fetchHelper`), the canonical form is `@utils/api/fetchHelper`. For other lib subdirs (`@lib/assets/...`, `@lib/independence/...`, `@lib/broker`), either alias works. Also flag `@lib/utils/...` as 🟠 — double-`utils`, resolves to nothing. CodeRabbit gets this wrong and routinely suggests `@lib/api/...` — reject those suggestions.
