...
class EventProducerTest {
    ...
    fun setUp() {
        ...
        val setKafkaMethod = EventProducer::class.java.getDeclaredMethod("setKafkaCaProducer", KafkaTemplate::class.java)
    ...
    fun `should publish dividend event when valid dividend market data is provided`() {
        ...
        val dividendMarketData = MarketData(
            asset = testAsset,
            source = "TEST",
            priceDate = LocalDate.now(),
            close = BigDecimal("150.00"),
    ...
    fun `should publish split event when valid split market data is provided`() {
        ...
        val splitMarketData = MarketData(
            asset = testAsset,
            source = "TEST",
            priceDate = LocalDate.now(),
            close = BigDecimal("150.00"),
    ...
    fun `should not publish event when market data has no dividend or split`() {
        ...
        val regularMarketData = MarketData(
            asset = testAsset,
            source = "TEST",
            priceDate = LocalDate.now(),
            close = BigDecimal("150.00"),
    ...
    fun `should handle both dividend and split in same market data`() {
        ...
        val combinedMarketData = MarketData(
            asset = testAsset,
            source = "TEST",
            priceDate = LocalDate.now(),
            close = BigDecimal("150.00"),
